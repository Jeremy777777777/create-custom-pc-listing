...
用途只影响辅助图片的场景、图标、文案语气与光效。仍须遵守 `image-spec.md` 的槽位顺序和背景分布。主图永远不因用途而改成倾斜、三分之四角度或场景背景，也永远不添加额外 Logo。对允许展示原机品牌的 customized laptop/desktop 图片，PT 图默认使用与已验证底机制造商一致的 OEM Logo（例如 HP 机型使用 HP Logo），不能用卖家 Logo 替代、混用或猜测品牌；具体合成按第 4 节执行。
...
7. 先完成并保存无品牌 PT01–PT08 母版，再用原始 Logo 文件进行确定性后处理；禁止让 ImageGen 重画 Logo、品牌文字或商标。每张图单独选择负空间位置，不设固定右下角。Logo 的可见像素、白色 keyline、背景牌及其安全留白都不得覆盖或接触产品、标题、正文、规格卡、脚注、接口标注、引导线、边框或装饰线。没有合格安全区时宁可不加 Logo，并把该槽位标记为待人工处理。
8. 在产品目录保存 `logo-placement.json`，逐图记录 `x`、`y`、`width`、`height` 与样式。先输出到独立 review 目录并逐张检查；只有用户批准后，才用 `scripts/add-brand-badge.ps1` 从 `unbranded/` 重建并替换最终文件。不得在已带 Logo 的图上再次叠加。
...
- 逐张检查分辨率、比例、清晰度、裁切、颜色/角度、文字、Logo/版权、配件、接口、跨图规格一致性和 Amazon 主图限制。Logo 必须与底机 OEM 相符，使用官方或已获准原始资产，不能拼错、变形、擅自改色、重绘，也不能覆盖或接触文字、线条、接口、产品和信息卡。检查应包含 100% 尺寸视觉复核，而不仅是缩略图。
...
`image-manifest.md` 逐槽位记录实际路径、状态、授权素材来源、产品事实来源及必要元数据说明。如使用 OEM Logo，还要记录核实后的制造商、Logo 文件路径与来源、允许使用的 listing 类型、合成脚本、`logo-placement.json` 和逐图验收结果。只有文件存在并通过事实、版权、尺寸、内容和合规检查时，状态才可为 `VERIFIED`。缺图时不得伪造路径。
